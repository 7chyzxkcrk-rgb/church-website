...
export default function Header() {
  ...
  return (
    <div>
      <div className="bg-rose-300 text-white text-center py-2 text-xs md:text-sm tracking-widest">
      ...
      <div className="relative" style={{
        ...
        <div className="relative z-10 text-center py-8 md:py-16 px-4">
          <h1 className="text-3xl md:text-5xl font-bold text-teal-300" style={{fontFamily: 'serif'}}>여호와는 나의 목자시니</h1>
          <p className="text-white mt-4 text-xs md:text-sm">여호와는 나의 목자시니 내게 부족함이 없으리로다</p>
      ...
      <nav className="bg-white border-t border-b border-gray-200">
        <ul className="hidden md:flex justify-center gap-8 py-3 text-sm font-medium text-gray-700">
        ...
        <ul className="flex md:hidden justify-around py-3 text-xs font-medium text-gray-700">
          {['교회안내', '예배와 영성', '교회학교', '전도와 선교', '교회소식', '수정갤러리'].map(item => (
            <li key={item} className="cursor-pointer hover:text-teal-500 transition-colors text-center">{item}</li>
          ))}
        </ul>
      </nav>
    </div>
